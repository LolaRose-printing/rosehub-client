...
// Update the onSubmit function in your component
const onSubmit: SubmitHandler<ServiceInputs> = async (data) => {
  ...
  try {
    ...
    // Get the token directly from the auth context if available
    let token = user?.accessToken; // Adjust this based on your useAuth implementation
    
    // If not available in context, try to get it from localStorage or cookies
    if (!token && typeof window !== 'undefined') {
      // Check if your auth system stores token in localStorage
      token = localStorage.getItem('accessToken');
      
      // Or check cookies if that's where your auth system stores it
